...
const { Facility, Department, ServiceCategory, Service, FacilityService } = require('../models');
...
const NAME_INCLUDES = [
  ...
  { model: ServiceCategory, attributes: ['serviceCategoryId', 'serviceCategoryName'] },
...
function toResponse(facilityServiceRow) {
  ...
  return {
    ...
    serviceCategoryName: plain.ServiceCategory ? plain.ServiceCategory.serviceCategoryName : null,
    status: plain.status,
...
class FacilityServiceCatalogService {
  async getList({ page, limit, search, status, facilityId, serviceCategoryId, tenantUuid }) {
