...
const { ORGANIZATION_TYPES, STATUSES } = require('../validations/organization.validation');
...
const { SERVICE_CATEGORIES } = require('../validations/facilityService.validation');
...
const definition = {
  ...
  info: {
    ...
    description:
      ...
      'Every route below `' + basePath + '` (other than `/health`) requires a bearer ' +
  ...
  tags: [
    ...
    { name: 'Organizations', description: 'Health networks / authorities — top of the org hierarchy' },
    { name: 'Facilities', description: 'CHC / PHC / District Hospital / Clinic, belonging to an organization' },
    { name: 'Departments', description: 'OPD / IPD / Lab / Pharmacy / etc., within a facility' },
    { name: 'Facility Services', description: 'Services offered at a facility, optionally scoped to a department' },
    { name: 'Service Categories', description: 'Categories used to classify facility services, scoped to an organization' },
    { name: 'Internal', description: 'Service-to-service routes callable only with the internal service token' },
  ],
  components: {
    securitySchemes: {
      bearerAuth: {
    ...
    parameters: {
      ...
      SearchParam: {
        name: 'search',
        in: 'query',
        description: 'Free-text search, matched against the resource name (and code, where one exists).',
    ...
    schemas: {
      ...
      Organization: {
        ...
        properties: {
          ...
          organizationName: { type: 'string', example: 'State Health Department' },
          organizationCode: { type: 'string', example: 'STATE-HEALTH-DEPARTMENT-9F3A1B2C' },
          organizationType: { type: 'string', nullable: true, enum: [...ORGANIZATION_TYPES, null] },
      ...
      Facility: {
        ...
        properties: {
          ...
          facilityType: { type: 'string', nullable: true, enum: [...FACILITY_TYPES, null] },
      ...
      Department: {
        ...
        properties: {
          ...
          departmentType: { type: 'string', nullable: true, enum: [...DEPARTMENT_TYPES, null] },
          status: { type: 'string', enum: [...STATUSES] },
          ...auditFields,
        },
      },
      ...
      FacilityServiceCatalog: {
        ...
        properties: {
          ...
          serviceCategory: { type: 'string', nullable: true, enum: [...SERVICE_CATEGORIES, null] },
          status: { type: 'string', enum: [...STATUSES] },
          ...auditFields,
        },
      },
      ...
      ServiceCategoryCreateRequest: {
        ...
        properties: {
          organizationId: { type: 'integer' },
          serviceCategoryName: { type: 'string', minLength: 2, maxLength: 200 },
    ...
    responses: {
      ...
      NotFound: {
        description: 'No resource with that id exists for the caller\u2019s tenant.',
        content: { 'application/json': { schema: { $ref: '#/components/schemas/Error' } } },
      },
    },
  },
...
const options = {
  definition,
  // JSDoc `@swagger` blocks live next to the routes they describe.
  apis: [
    `${__dirname}/../app.js`,
    `${__dirname}/../routes/*.js`,
  ],
